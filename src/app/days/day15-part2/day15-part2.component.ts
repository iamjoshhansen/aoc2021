...
import { Position, normalize, Canvas, Grid2D, toInt } from 'src/app/utils';

@Component({
  selector: 'app-day15-part2',
  templateUrl: './day15-part2.component.html',
  styleUrls: ['./day15-part2.component.scss'],
})
export class Day15Part2Component extends Star implements OnInit {
  ...
  async solve(input: string) {
    const inputRisks = input.split('\n').map((row) => row.split('').map(toInt));
    const inputRiskGrid = new Grid2D(inputRisks);

    const risks: number[][] = [];
    const newWidth = inputRiskGrid.width * 5;
    const newHeight = inputRiskGrid.height * 5;

    for (let y = 0; y < newHeight; y++) {
      risks.push([]);
      const yCol = Math.floor(y / inputRiskGrid.height);
      for (let x = 0; x < newWidth; x++) {
        const xCol = Math.floor(x / inputRiskGrid.width);
        const inputRisk = inputRiskGrid.getValue({
          x: x % inputRiskGrid.width,
          y: y % inputRiskGrid.height,
        })!;
        risks[risks.length - 1].push(((inputRisk - 1 + xCol + yCol) % 9) + 1);
      }
    }
    ...
    const canvas = new Canvas(this.canvasRef.nativeElement, (size) => ({
      ...
      width: riskGrid.width,
      height: riskGrid.height,
    }));
    ...
    const explorers = new Heap<Explorer>((a, b) => a.totalRisk - b.totalRisk);
    explorers.push(new Explorer(source));
    ...
    while (explorers.size() > 0 && !foundTarget) {
      const bestExplorer = explorers.pop();
    ...
    ctx.moveTo(
      (source.x + 0.5) * cellWidth - offset,
      (source.y + 0.5) * cellHeight - offset
    );
    path.forEach((cell) => {
      ctx.lineTo(
        (cell.x + 0.5) * cellWidth - offset,
        (cell.y + 0.5) * cellHeight - offset
      );
    });
